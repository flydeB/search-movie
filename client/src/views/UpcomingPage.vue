<template>
  <div class="up-page">
    <div class="page-header">
      <h2 class="page-title">即将上映</h2>
      <p class="page-subtitle">近期即将上映的电影</p>
    </div>
...
/**
 * 即将上映页
 * 展示近期即将上映的电影，数据来源 TMDB
 * 支持分页浏览和电影详情查看
 */
import { ref, onMounted } from 'vue'
import { getUpcoming, getMovieDetail } from '../api/movie'
...
async function fetchMovies(page?: number) {
  ...
  try {
    ...
    const result = await getUpcoming(p, activeRegion.value)
    movies.value = result.movies
    totalPages.value = result.totalPages
    totalResults.value = result.totalResults
  } catch (e: any) {
    console.error('即将上映请求失败:', e)
    movies.value = []
  } finally {
    loading.value = false
  }
...
<style scoped>
.up-page { min-height: 100vh; padding-bottom: 80px; }

.page-header { text-align: center; padding: 48px 24px 8px; }
...
.result-info { display: flex; align-items: center; justify-content: space-between; }

.result-left { display: flex; align-items: center; gap: 12px; }

.result-count { padding: 4px 14px; background: var(--primary-dim); border-radius: 20px; color: var(--primary); font-size: 13px; font-weight: 600; }

.result-page { font-size: 13px; color: var(--text-secondary); }

.result-source { padding: 4px 14px; background: rgba(100, 180, 120, 0.1); border-radius: 20px; color: #6cbe7a; font-size: 12px; font-weight: 500; }
...
.loading-dots span { width: 8px; height: 8px; border-radius: 50%; background: var(--primary); opacity: 0.3; animation: dot-bounce 1.4s ease-in-out infinite; }
...
.movie-grid { max-width: 1400px; margin: 32px auto 0; padding: 0 24px; display: grid; grid-template-columns: repeat(auto-fill, minmax(185px, 1fr)); gap: 24px; }

.pagination-wrap { display: flex; justify-content: center; margin-top: 48px; padding: 0 24px; }

/* 分页暗色主题 */
:deep(.ant-pagination-item) { background: var(--bg-card) !important; border-color: var(--border-color) !important; border-radius: 8px !important; }
...
:deep(.ant-pagination-jump-next button) { background: var(--bg-card) !important; border-color: var(--border-color) !important; color: var(--text-body) !important; border-radius: 8px !important; }
:deep(.ant-pagination-prev:hover button),
:deep(.ant-pagination-next:hover button) { border-color: rgba(232, 183, 74, 0.3) !important; color: var(--primary) !important; }
:deep(.ant-pagination-item-ellipsis) { color: var(--text-secondary) !important; }
:deep(.ant-pagination-options-quick-jumper input) { background: var(--bg-card) !important; border-color: var(--border-color) !important; color: var(--text-primary) !important; border-radius: 8px !important; }
:deep(.ant-pagination-options-quick-jumper input:focus) { border-color: var(--primary) !important; box-shadow: 0 0 0 2px rgba(232, 183, 74, 0.1) !important; }
