...
│   ├── Dockerfile              # CloudRun 容器构建：Node 20-alpine + HEALTHCHECK
│   ├── entrypoint.sh           # 容器启动脚本：调试日志输出
│   ├── .dockerignore           # Docker 构建忽略规则
│   ├── cloudbaserc.json        # CloudRun 部署配置
│   └── src/
...
| 配置项 | 值 | 说明 |
|--------|-----|------|
| 端口 | `3000` | 通过环境变量 `PORT` 设置 |
| 镜像 | `node:20-alpine` | 官方 Node.js 20 Alpine 镜像 |
| 启动命令 | `node dist/index.js` | 运行编译后的产物 |
| 探针延迟 | `InitialDelaySeconds: 15` | 必须设为 15s，否则容器启动失败 |
| 最小实例 | `MinNum: 1` | 保持至少 1 个实例运行 |
